...
* **Порядок байт:** Все числовые данные в WMO файлах хранятся в **Little Endian** порядке.
* **Структура чанков:** Файлы состоят из последовательности "чанков" (chunks). У каждого чанка есть заголовок и блок данных.

  **Заголовок чанка (8 байт):**

  ```cpp
  struct ChunkHeader {
      char     chunk_id[4];        // Идентификатор чанка. Хранится в обратном порядке (например, 'MVER' в коде будет 'REVM' в файле).
      uint32_t chunk_data_size;  // Размер данных чанка (в байтах), следующих *после* этого заголовка.
  };
  ```

  **Данные чанка:**
  Сразу после заголовка следует блок данных размером `chunk_data_size`.

## Типы WMO файлов

WMO-объекты состоят из двух типов файлов: корневого (`WMOName.wmo`) и одного или нескольких файлов групп (`WMOName_NNN.wmo`).
Корневой файл является "схемой" всего объекта: он определяет общую структуру, содержит ссылки на используемые модели (`.m2`), текстуры, материалы и, самое главное, на файлы групп, из которых состоит геометрия.
Файлы групп, в свою очередь, содержат фактическую 3D-геометрию (вершины, полигоны) для отдельных частей WMO.
Далее мы подробно рассмотрим структуру сначала корневого WMO, а затем файла группы.
...
* **Структура (4 байта данных + 8 байт заголовок чанка):

    ```cpp
    struct MVER_Chunk {
        char     chunk_id[4];        // "REVM" (MVER)
        uint32_t chunk_data_size;  // Размер данных, всегда 4
        uint32_t version;            // Версия формата WMO (17) для WotLK
    };
    ```
...
* **Структура (64 байта данных + 8 байт заголовок чанка):
    ...
    struct MOHD_Data_WotLK {
        ...
        uint32_t nGroups;          // Количество групп (MOGI и файлов _NNN.wmo) ЧИСЛО обозначает количество групп, которые будут загружены из файлов _NNN.wmo
        ...
        CAaBox   bounding_box;     // РАЗМЕР .WMO и всех его внутренностей (6 float: minX,Y,Z, maxX,Y,Z)
...
* **Расположение:** После чанка `MOMT`.
* **Назначение:** Содержит блок имен для каждой группы WMO: "Boss", "Hall", "Corrido". Не нужно для navmesh, но нужно для отладки.
* **Структура данных:** `MOGN` — это единый блок данных, содержащий последовательность строк, каждая из которых заканчивается нулевым символом (`\0`). Ссылки на имена из других чанков (например, `MOGI`) представляют собой **смещение в байтах** от начала данных `MOGN`. У чанка нет строгой структуры C++, его можно представить так:

    ```cpp
    // Псевдо-структура MOGN
    struct MOGN_ChunkData {
        // Непрерывный массив символов, содержащий все имена.
        // Каждое имя отделено от следующего нулевым символом.
        char names_blob[]; // "Name1\0Name2\0AnotherGroupName\0..."
    };
    ```
...
* **Структура данных:** Массив структур `SMOGroupInfo`.
    ...
    struct SMOGroupInfo_WotLK {
        ...
        int32_t  name_offset;       // Смещение в байтах от начала данных чанка MOGN до имени этой группы.
                                    // Если значение -1, имя отсутствует.
    };
    ```

* **Пример связи MOGN и MOGI:**
  Предположим, у нас есть `MOGN`, который содержит строки `"hall01\0boss_room\0corridor\0"`.
  Если для Группы 5 поле `name_offset` в `MOGI` равно `7`, это означает, что ее имя начинается с 7-го байта данных `MOGN`, что указывает на строку `"boss_room"`.
