...
void MainWindow::onTreeItemDoubleClicked(QTreeWidgetItem *item, int /*column*/)
{
    ...
    try
    {
        ...
        QMessageBox::information(this, "Телепортация",
                                 QString("Начинаю телепортацию в точку '%1'... "
                                         "Пожалуйста, не двигайте персонажа.")
                                     .arg(item->text(0)));
        ...
        if (success)
        {
            QMessageBox::information(this, "Успех", "Телепортация завершена!");
        }
        else
        {
