...
struct PlayerData
{
    ...
    uint32_t Health = 0;
    uint32_t maxHealth = 0;
    /** @brief Текущее значение маны. */
    uint32_t Mana = 0;
    /** @brief Максимальное значение маны. */
    uint32_t maxMana = 0;
    /** @brief Текущее значение ярости. Уже разделено на 10 в DLL (т.е. 0-100). */
    uint32_t Rage = 0;
    /** @brief Текущее значение энергии (0-100). */
    uint32_t Energy = 0;
    /** @brief Текущее значение силы рун. Уже разделено на 10 в DLL (т.е. 0-100). */
    uint32_t RunicPower = 0;
    uint8_t level = 0;
    uint32_t flags = 0;
    uint64_t targetGuid = 0;
    uint64_t autoAttackTargetGuid = 0;
    ...
    /**
     * @brief Битовая маска, представляющая состояние готовности рун Рыцаря Смерти.
     * @details Каждый бит с 0 по 5 соответствует одной руне. 1 - руна готова, 0 - на перезарядке.
     *          Это значение читается напрямую из памяти клиента и обновляется DLL.
     */
    uint32_t runeStatusMask;
};
