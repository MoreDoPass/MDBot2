...
def visualize_with_pyvista(vertices, faces=None):
    ...
    # --- Добавление двух тестовых сфер для проверки координат ---
    # Первая сфера (пурпурная)
    sphere1_pos = (-566.1228, 767.08593, -211.9894)
    sphere1 = pv.Sphere(radius=1, center=sphere1_pos) # Радиус побольше для заметности
    plotter.add_mesh(sphere1, color='magenta', style='surface')

    # Вторая сфера (голубая)
    sphere2_pos = (-593.5464, 2064.0444, 132.94216)
    sphere2 = pv.Sphere(radius=1, center=sphere2_pos) # Радиус побольше для заметности
    plotter.add_mesh(sphere2, color='cyan', style='surface')
...
if __name__ == '__main__':
    ...
    adt_dir = "fast_test/terrain_working/ADT"
